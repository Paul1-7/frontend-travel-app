...
import DataTablesButtons from './DataTableButtons'
...
const DataTable = ({
  ...
  const emptyRows =
    page >= 0 ? Math.max(0, (1 + page) * rowsPerPage - dataFiltered.length) : 0
  ...
  return (
    <Box sx={{ marginTop: '16px' }}>
      ...
      <TableContainer>
        <Table
          sx={{ minWidth: width || 750, height: 350 }}
            ...
            {stableSort(dataFiltered, getComparator(order, orderBy))
              ...
              .map((row, index) => (
                <Fragment key={index}>
                  <TableRow
                    hover
                    sx={{ height: 55, '& > *': { borderBottom: 'unset' } }}
                  >
                    {collapse && (
                      <TableCell>
                        <IconButton
                  ...
                  {collapse && (
                    <TableRow>
                      ...
                      >
                        <Collapse
                          in={open.state && open.index === index}
                          timeout="auto"
                        >
                          <Box sx={{ margin: 1 }}>
                            <Table size="small" aria-label={collapse + index}>
                              <TableHead>
                                <TableRow>
                                  {Object.keys(row?.[collapse]?.[0] ?? {}).map(
                                    (header, index) => (
                                      <TableCell key={index}>
                                        {header}
                                      </TableCell>

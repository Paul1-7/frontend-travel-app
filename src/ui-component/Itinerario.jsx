...
const Itinerario = ({ generateMarkers, deleteMarkers, data = [] }) => {
  ...
  const getIdLugaresUnicos = () => {
    return idLugares
      .filter(
        ({ name }, index, a) => a.findIndex((e) => name === e.name) === index
      )
      .map(({ value }) => value)
  }
  ...
  useEffect(() => {
    ...
    const lugaresUnicos = resGet.filter((item) =>
      idLugaresUnicos.includes(item.id)
    )

    const markers = lugaresUnicos.map(({ lng, lat, nombre }) => ({
      lngLat: [lng, lat],
      label: nombre
    }))
  ...
  useEffect(() => {
    if (!data.length || !resGet.length) return

    data.forEach(({ idLugar, Itinerario, horaInicio, horaFin }, index) => {
      // console.log('TCL: {I}', {I});
      if (index === 0) {
        update(index, {
          idLugar,
                  ...
                  >
                    ...
                    >
                      ...
                      <Grid item xs={12} md={6}>
                        <Input
                          label="Descripcion"
                          name={`itinerarios.${index}.descripcion`}
                          isArray
                        />
                      </Grid>
                      <Grid item xs={12} md={6}>
                        <Input
                          ...
                          name={`itinerarios.${index}.horaInicio`}
                          isArray
                        />
                      </Grid>
                      <Grid item xs={12} md={6}>
                        <Input
                          label="Hora fin"
                          name={`itinerarios.${index}.horaFin`}
                          isArray
                        />
                      </Grid>
                    </Grid>
